 ...
 esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
 {
     ...
     if (config->tear_cfg.te_gpio_num > 0) {
         ...
         if (config->tear_cfg.task_affinity < 0) {
             res = xTaskCreateWithCaps(bsp_display_sync_task, "Tear task", config->tear_cfg.task_stack, tear_ctx, config->tear_cfg.task_priority, NULL, MALLOC_CAP_SPIRAM);
         } else {
             res = xTaskCreatePinnedToCoreWithCaps(bsp_display_sync_task, "Tear task", config->tear_cfg.task_stack, tear_ctx, config->tear_cfg.task_priority, NULL, config->tear_cfg.task_affinity, MALLOC_CAP_SPIRAM);
         }
